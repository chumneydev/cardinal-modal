...
(function ($) {
    ...
    Cardinal.prototype.ajaxLoadContent = function () {
        ...
        if (_.options.ajaxURL != '') {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    var modalClass = _.options;

                    modalClass.innerHTML = xhr.responseText;

                    console.log(xhr.responseText);
                }
            };
            xhr.open('GET', _.options.ajaxURL);
            xhr.send();


            /*_.options.content = $.get(_.options.ajaxURL, function(data) {
                $('.ca-modal-content').innerHTML = data
    ...
    function buildOut() {
        ...
        } else if (typeof this.options.content === xhr) {
            content = this.options.content.innerHTML = xhr.responseText;
        } else {
        ...
        // Create header if title or subtitle is not provided
        if (this.options.title != '' || this.options.subTitle != '') {
            this.modalHeader = document.createElement("div");
            this.modalHeader.className = "ca-modal-header";
            this.modal.appendChild(this.modalHeader);

            if (this.options.headerColor !== null) {
                this.modalHeader.setAttribute('style', 'background:' + this.options.headerColor);
            }
            ...
            if (this.options.title != '') {
                ...
                createTitle = document.createTextNode(this.options.title);
                title.appendChild(createTitle);
            }
            
            if (this.options.subTitle != '') {
                subTitle = document.createElement("h4");
                this.modalHeader.appendChild(subTitle);

                createSubTitle = document.createTextNode(this.options.subTitle);
                subTitle.appendChild(createSubTitle);
            }
